...
"""
        ...
        @pw.udf
        def parse_doc(data: str, metadata, page_number : int = None , level : int = None , id_ : str = None) -> list[pw.Json]:
            data = data.encode("utf-8")
            rets = _coerce_sync(self.parser)(data)
            metadata = metadata.value
            if page_number is None:
                return [dict(text=ret[0], metadata={**metadata, **ret[1], "level": level , "id_" : id_}) for ret in rets]
            else:
                return [dict(text=ret[0], metadata={**metadata, **ret[1], "page_number" : page_number}) for ret in rets]  # type: ignore
        
        try:
            parsed_docs = docs.select(data=parse_doc(docs.data, docs._metadata, docs.page_number)).flatten(
                pw.this.data
            )
        except :
          parsed_docs = docs.select(data=parse_doc(docs.data, docs._metadata, docs.level, docs.id_)).flatten(
                pw.this.data
            )
        ...
        def split_doc(data_json: pw.Json) -> list[pw.Json]:
            ...
            rets = _coerce_sync(self.splitter)(text)
            return [
